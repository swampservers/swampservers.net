...
<?=Page("News", "/s/screenshots/newscast.jpg")?>
...
foreach ($stuff as $post) {
	...
	<div>
		...
		<?php foreach ($post['reactions'] as $reaction) {?>
			...
			<div class="btn btn-xs btn-default" style="cursor:default;">
			<?php
if (substr($reaction[1], 0, 4) == "http") {
		...
		<div class="row">
			<p class="text-muted pull-right" style="position:relative;top:16px;">
				...
				in
				<a class="color hoverul" onclick='discordgroup()' href="/discord">#<?=htmlspecialchars($post["channel"])?></a>
				on
...
<div class="well">
	...
	<ol>
		...
		<li>Don't be excessively toxic</li>
		<li>No shock videos/images</li>
		<li>Do not be a skid</li>
	</ol>
...
<div class="text-center"><strong>Richest players</strong></div>
	...
	<?php
$stuff = SQL_Query("SELECT points FROM users WHERE id!=0 ORDER BY points DESC LIMIT 5", []);
...
<?php
	...
	if (!Login()) {
		$msg = "Login to submit!";
	} elseif (redis()->exists("user:".Login()->id.":thought_added")) {
		$msg = "Submit another tomorrow!";
	} elseif ($_SERVER['REQUEST_METHOD'] == 'POST') {
		$thought = trim(str_replace("\n", "", $_POST["thought"] ?? ""));
		if (strlen($thought) < 2 || strlen($thought) > 100) {
			$msg = "Invalid thought!";
		} else {
			$msg = 'Successfully added "'.$thought.'"!';
			redis()->setex("user:".Login()->id.":thought_added", (-time()-date("Z")) % 86400 + 86400, $thought);
			try_add_thought(htmlspecialchars($thought));
		}
	}
?>
...
<div class="well">
	...
	<div class="text-center" style="margin:4px auto;">
		<?php if ($msg) {?>
			<div class="text-center"><strong><?=htmlspecialchars($msg)?></strong></div>
		<?php } else {?>
...
foreach (["/swamp/gmod", "/swamp/gmod/repos/restricted", "/swamp/gmod/repos/contrib"] as $repo) {
...
foreach ($junk as $x) {
    $x = explode(":", $x, 2);
    if ($x[1] == "Lint" || $x[1] == "glualint" || strtolower($x[1]) == "merge") {continue;}
