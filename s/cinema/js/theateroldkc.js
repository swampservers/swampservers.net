	...
	var FileVideo = function() {
		...
		*/
			...
			if ( (this.player !== null) && (this.player.getDuration() > -1) ) {
				...
				if ( this.propertime < this.player.getDuration() && (( this.player.getPosition() + maxoffset < this.propertime ) || ( this.player.getPosition() - maxoffset > this.propertime ))) {
					...
					if((document.getElementById("loadingmessage")===null) && (this.videoId.indexOf("rtmp://") == -1) && (this.videoId.indexOf(".smil") == -1)) {
						...
						div.innerHTML = "<img src=\"http://i.imgur.com/O0OGByN.gif\" style=\"width:150px;height:150px;\" alt=\"LOADING\">";
				...
				//if((this.player.getDuration()==-1) && (this.videoId.indexOf("/vod/") != -1) && (this.videoId.indexOf("rtmp://") != -1)) {
						/*
						var element = document.getElementById("loadingmessage");
						if(element) {element.parentNode.removeChild(element);}
						var div = document.createElement("div");
						div.id = "loadingmessage";
						div.innerHTML = "Problem loading video!<br>Hold Q and try refreshing the theater.";
						document.getElementById("player-container").insertBefore(div,document.getElementById("player_wrapper"));
						*/
					//this.setVideo(this.videoId); //will make a shitty loop
					//this.setStartTime(this.propertime+0.5); //IT DOESN'T QUITE WORK BUT WHATEVER
				//}

				//document.getElementById("loadingmessage").innerHTML = Math.round(this.player.getPosition()) + " / " + Math.round(this.propertime);
			}
		...
		this.onReady = function() {
			...
			//document.getElementById("vid_logo").style.display = "none";
